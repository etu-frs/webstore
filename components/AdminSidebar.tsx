...
export const AdminSidebar: React.FC<AdminSidebarProps> = ({ 
  ...
  const sidebarTranslateClass = !isDesktop ? (isMobileOpen ? 'translate-x-0' : '-translate-x-full') : 'md:translate-x-0';


  return (
    <aside 
      id="admin-sidebar"
      className={`fixed top-20 left-0 bottom-0 z-40 bg-gumball-dark text-white p-4 sm:p-6 space-y-4 rounded-r-xl shadow-lg 
              ...
              <li key={section.id} className="mb-1 sm:mb-2">
                ...
                >
                  {section.icon && React.cloneElement(section.icon as React.ReactElement<{ className?: string }>, { 
                    className: `w-5 h-5 sm:w-6 sm:h-6 flex-shrink-0 ${!showTextLabels && isDesktop ? 'mr-0' : 'mr-2 sm:mr-3'}` 
                  })}
      ...
      <div className="mt-auto pt-4 space-y-2 border-t border-gumball-yellow/30">
        {authCtx?.isAdminAuthenticated && (
            <Button
                ...
                title={!showTextLabels ? "Logout" : undefined}
            >
                <LogoutIcon className={`flex-shrink-0 ${!showTextLabels && isDesktop ? 'w-5 h-5' : 'w-5 h-5 mr-2'}`} /> 
