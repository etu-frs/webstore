...
export const Button: React.FC<ButtonProps> = ({
  ...
  const commonContent = (
    <>
      ...
      {leftIcon && !isLoading && <span className="mr-2 group-hover:animate-wiggleSoft inline-block">{leftIcon}</span>}
      {children}
      {rightIcon && !isLoading && <span className="ml-2 group-hover:animate-wiggleSoft inline-block">{rightIcon}</span>}
    </>
  );
